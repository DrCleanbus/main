...
const defaultCompanyId = process.env.GHL_COMPANY_ID;
const defaultLocationId = process.env.GHL_LOCATION_ID;
if (!token)
  throw new Error(
    "GHL_ACCESS_TOKEN must be supplied by the connector secret store",
  );
const api = new HighLevelClient(token, process.env.GHL_BASE_URL);
    ...
    try {
      return response(
        await api.searchUsers({
          ...args,
          companyId: required(args.companyId ?? defaultCompanyId, "companyId"),
          locationId: required(
...
server.registerTool(
  "list_calendars",
  {
    description: "List calendars and team members in the authorized location.",
    inputSchema: {
      locationId: z.string().optional(),
