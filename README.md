...
Public tools are `search_users`, `list_calendars`, `get_calendar`, `update_calendar`, `ensure_user_on_matching_calendars`, and `update_calendar_service_time`. `update_calendar_service_time` accepts `calendarId`, expected and proposed durations (each with a `minutes` or `hours` unit), `dryRun`, and `changeReason`. It checks the current value before writing and verifies real updates with a second read. Use `dryRun: true` to preview normalized whole-minute values. The reason is returned for the invoking agent's audit trail; HighLevel does not persist it as calendar configuration.
